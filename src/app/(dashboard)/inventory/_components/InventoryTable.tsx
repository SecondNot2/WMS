...
export function InventoryTable() {
  ...
  const columns = [
    {
      header: "Sản phẩm",
      accessor: "name",
      render: (val: unknown, row: StockRow) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="text-[11px] font-mono text-text-secondary uppercase tracking-tighter">
              {row.sku}
            </span>
            <Link
              href={`/products/${row.id}`}
              className="text-sm font-bold text-text-primary hover:text-accent transition-colors truncate max-w-50"
            >
              {val as string}
            </Link>
          </div>
        ...
        <div className="text-right">
          <p className="text-sm font-bold text-text-primary">
            {new Intl.NumberFormat("vi-VN").format(
              (val as number) * row.currentStock,
            )}{" "}
            đ
          </p>
          <p className="text-[10px] text-text-secondary">
            Giá nhập: {new Intl.NumberFormat("vi-VN").format(val as number)}
          </p>
        </div>
      ),
    },
    {
      ...
      render: (val: unknown) => (
        <div className="flex justify-end gap-2">
          <button
            className="p-2 hover:bg-background-app rounded-lg text-text-secondary hover:text-accent transition-all group"
            title="Lịch sử tồn kho"
          >
            <History className="w-4 h-4" />
          </button>
          <button
            className="p-2 hover:bg-background-app rounded-lg text-text-secondary hover:text-accent transition-all flex items-center gap-1 text-[11px] font-bold"
  ...
  return <DashboardTable title="" columns={columns} data={mockStock} />;
}
