...
export default function InventoryReportPage() {
  ...
  const handleExport = async () => {
    ...
    setExporting(true);
    try {
      const blob = await reportsApi.exportExcel({ type: "inventory" });
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `reports-inventory-${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  return (
    <div className="p-4 sm:p-5 space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-1 bg-card-white rounded-xl border border-border-ui shadow-sm p-4 sm:p-5">
