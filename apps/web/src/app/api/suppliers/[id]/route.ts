import { updateSupplierSchema } from "@wms/validations";
...
export const GET = handle<{ id: string }>(
  { roles: READ },
  async (_req, { params }) => ok(await service.getSupplierById(params.id)),
);
...
export const PATCH = handle<{ id: string }>(
  ...
  async (req, { params, user }) => {
    ...
    return ok(await service.updateSupplier(params.id, body, user!.id));
  },
);
...
export const DELETE = handle<{ id: string }>(
  ...
  async (_req, { params, user }) => {
    await service.deleteSupplier(params.id, user!.id);
