...
export function ProductImportWizardConnected() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Không thể nhập sản phẩm"));
    }
  };
    ...
    <div className="bg-card-white rounded-2xl border border-border-ui shadow-sm overflow-hidden w-full">
      <div className="p-8 space-y-6">
        <div className="aspect-video bg-background-app/50 border-3 border-dashed border-border-ui rounded-3xl flex flex-col items-center justify-center text-center">
          ...
          <h4 className="text-lg font-bold text-text-primary">Tải file sản phẩm lên hệ thống</h4>
          <p className="text-sm text-text-secondary mt-1">Hỗ trợ file .xlsx hoặc .xls</p>
          ...
          {file && <p className="text-xs text-text-secondary mt-3">Đã chọn: {file.name}</p>}
        </div>

        <div className="flex items-center gap-4 p-4 bg-info/5 border border-info/20 rounded-2xl text-info">
          <AlertCircle className="w-5 h-5 shrink-0" />
          <p className="text-[12px] leading-relaxed">
            File cần có các cột tối thiểu: SKU, Tên sản phẩm, Đơn vị, Mã danh mục. Các dòng lỗi sẽ được bỏ qua và trả về lý do.
          </p>
        </div>

        {error && <div className="p-4 rounded-xl bg-danger/5 border border-danger/20 text-sm text-danger">{error}</div>}
        ...
        {result && (
          <div className="p-5 rounded-xl bg-success/5 border border-success/20 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
              <div className="bg-card-white border border-border-ui rounded-lg p-3">Imported: <b>{result.imported}</b></div>
              <div className="bg-card-white border border-border-ui rounded-lg p-3">Skipped: <b>{result.skipped}</b></div>
              <div className="bg-card-white border border-border-ui rounded-lg p-3">Errors: <b>{result.errors.length}</b></div>
            </div>
                ...
                <table className="w-full text-left text-xs">
                  ...
                  <tbody>
                    {result.errors.map((item) => (
                      <tr key={`${item.row}-${item.reason}`} className="border-t border-border-ui">
