...
function productData(
  input: CreateProductSchemaInput | UpdateProductSchemaInput,
) {
  return {
...
async function assertCategoryExists(categoryId: string) {
  ...
  if (!category)
    throw new AppError("VALIDATION_ERROR", "Danh mục không tồn tại");
}
  ...
  for (const [index, row] of rows.entries()) {
    ...
    try {
      const input: CreateProductSchemaInput = {
        ...
        barcode:
          cellString(row, ["barcode", "Barcode", "Mã vạch", "Ma vach"]) ?? null,
        name:
          cellString(row, ["name", "Name", "Tên sản phẩm", "Ten san pham"]) ??
          "",
        brand:
          cellString(row, ["brand", "Brand", "Thương hiệu", "Thuong hieu"]) ??
          null,
        model: cellString(row, ["model", "Model"]) ?? null,
        description:
          cellString(row, ["description", "Description", "Mô tả", "Mo ta"]) ??
          null,
        ...
        costPrice:
          cellNumber(row, ["costPrice", "Cost price", "Giá vốn", "Gia von"]) ??
          null,
        salePrice:
          cellNumber(row, ["salePrice", "Sale price", "Giá bán", "Gia ban"]) ??
          null,
        taxRate:
          cellNumber(row, ["taxRate", "Tax rate", "Thuế suất", "Thue suat"]) ??
          null,
        location:
          cellString(row, ["location", "Location", "Vị trí", "Vi tri"]) ?? null,
      };
