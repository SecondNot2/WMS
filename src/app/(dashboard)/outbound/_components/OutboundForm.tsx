...
export function OutboundForm({ initialData, isEdit }: OutboundFormProps) {
  ...
  const products = [
    { id: "p1", sku: "SP001", name: "Chuột không dây Logitech M331", unit: "Cái", price: 550000, stock: 45 },
    { id: "p2", sku: "SP002", name: "Bàn phím cơ Keychron K2", unit: "Cái", price: 1500000, stock: 12 },
    { id: "p3", sku: "SP003", name: "Lót chuột Razer Gigantus V2", unit: "Cái", price: 450000, stock: 120 },
    { id: "p4", sku: "SP004", name: "Tai nghe Sony WH-1000XM4", unit: "Cái", price: 5200000, stock: 5 },
  ];
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-text-primary mt-2">
            {isEdit ? "Chỉnh sửa phiếu xuất" : "Lập phiếu xuất kho"}
          </h1>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    {errors.purpose && <p className="text-xs text-danger mt-1">{errors.purpose.message}</p>}
                  </div>
                </div>
                
                <div className="space-y-2">
                  <label className="text-sm font-medium text-text-primary">Ghi chú thêm</label>
                  <textarea
                    {...register("note")}
                    rows={2}
                  ...
                  <table className="w-full text-left min-w-200">
                    <thead className="bg-[#f8fafc] border-b border-border-ui">
                      <tr>
                        ...
                        <th className="px-6 py-4 text-[11px] font-bold text-text-secondary uppercase tracking-wider">Sản phẩm / Tồn kho</th>
                        <th className="px-6 py-4 text-[11px] font-bold text-text-secondary uppercase tracking-wider w-32">Số lượng</th>
                        <th className="px-6 py-4 text-[11px] font-bold text-text-secondary uppercase tracking-wider w-40">Đơn giá xuất</th>
                    ...
                    <tbody className="divide-y divide-border-ui">
                      {fields.map((field, index) => {
                        ...
                        const isOverStock = selectedProduct && (watchItems?.[index]?.quantity || 0) > selectedProduct.stock;
                            ...
                            <td className="px-6 py-4">
                              ...
                              {selectedProduct && (
                                <div className={cn(
                                  ...
                                  {isOverStock ? <AlertCircle className="w-3 h-3" /> : <Info className="w-3 h-3" />}
                                  Tồn hiện tại: {selectedProduct.stock} {selectedProduct.unit}
                                </div>
                              )}
                            </td>
                            <td className="px-6 py-4">
                              <input
                                ...
                                className={cn(
                                  "w-full px-3 py-2 text-sm bg-white border rounded-lg outline-none focus:border-accent transition-all text-center",
                                  isOverStock ? "border-danger text-danger" : "border-border-ui"
                                )}
                              />
                            </td>
                            <td className="px-6 py-4">
            ...
            <div className="bg-white rounded-xl border border-border-ui shadow-sm p-6 sticky top-24">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center text-sm">
                  <span className="text-text-secondary font-medium">Số mặt hàng:</span>
              ...
              <div className="mt-6 p-4 bg-warning/5 rounded-lg border border-warning/10">
                <p className="text-[11px] text-warning leading-relaxed">
                  <strong>Lưu ý:</strong> Hệ thống sẽ kiểm tra tồn kho tại thời điểm phê duyệt. Nếu số lượng tồn kho không đủ, phiếu xuất sẽ bị từ chối tự động.
                </p>
              </div>
            </div>
          </div>
